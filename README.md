...
| **界面也是功能** | 玻璃拟态、浅色 / 深色、多种主题色、首页粒子动效。打开窗口的那一秒，就该知道这不是 2018 年的后台面板。 |
...
正式发布时推送与应用版本一致、以 `v` 开头的标签（例如 `v1.0.11`），工作流会在构建和测试成功后自动创建 GitHub Release，并把 Windows 安装包上传到 Release 资产。
