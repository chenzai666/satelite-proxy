...
2026-09-25（1.0.45 同版本修复）：AppStore.selected_nodes_by_subscription 按配置 ID 保存最后有效节点 ID，宽容读取器 store_from_json 必须恢复该字段。activate_subscription 切走前记忆，ensure_current_node_valid 优先保留有效当前节点，再恢复已启用配置的记忆，最后首节点兜底；无节点清空。手动/智能选点与内核自动选择同步记忆，不修改自动选择模式。删除、刷新清理失效引用；内核选择异步回包不得写入已禁用配置的节点。回归覆盖往返、序列化、改名、混合模式、刷新、删除及旧数据。
...
2026-09-24 节点页剪贴板快捷键：Ctrl/Cmd+C 对已选节点调用 `get_node_share_uri`，换行复制 v2rayN 可导入的分享 URI；任一节点不支持分享则整批不覆盖剪贴板。Ctrl/Cmd+V 经 `append_clipboard_nodes`（commands/subscription.rs → lib.rs → api.ts）向当前配置追加，不创建新配置：优先选中节点所属的唯一配置，否则当前代理节点所属配置，再否则唯一启用配置；目标不明确时拒绝。重复后端或同名节点保留独立 ID，名称追加 `-clone`、`-clone-2`。`AppStore.appended_node_ids` 持久记录本地追加节点，订阅刷新不覆盖/去重副本，删除节点/配置时清理。存储恢复同步读取本地覆盖和删除标记。只接收分享 URI / base64 URI，8MB 上限，不自动请求订阅 URL；自写配置仍只读，不自动切节点或启用配置，活动配置变更走既有防抖重建。输入框和弹窗不拦截快捷键；剪贴板凭据不输出到日志。
...
2026-09-24 分享链接 URI 编码：fragment 和 userinfo 不能直接使用表单编码的 `+` 表示空格；`share_component` 输出 `%20`，原有加号为 `%2B`，查询参数仍使用表单编码器。CI 名称往返测试曾捕获此错误，保留空格/加号/中文/特殊字符及凭据往返测试，不放宽名称一致性断言。
...
- `customNodes.sortNodes` 支持流式测速实时排序；保持 64px 行高及概览信息卡片在策略组上方。
