...
16. **`.srs` 规则集是 sing-box 专有** — Xray 生成器跳过用户自建远程 `.srs` 集（内置 3 条走 geosite 映射）；`srs.rs` decompile 固定用 sing-box 二进制。Xray 模式下 RulesPage 的内置 3 条显示为 geodata 来源（matcher + Loyalsoldier dat 信息），"更新"走 `refresh_geodata` 重下 `.dat` 而非 `.srs`。
