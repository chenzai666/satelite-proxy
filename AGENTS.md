...
2026-09-24 节点页剪贴板快捷键：Ctrl/Cmd+C 对已选节点逐条调用既有 `get_node_share_uri`，换行复制 v2rayN 可导入的分享 URI；任一节点不支持分享则整批不覆盖剪贴板。Ctrl/Cmd+V 只接收节点分享 URI 列表或 base64 URI 列表，复用 `add_subscription_text` 解析和入库；普通网站/订阅 URL 不自动请求。已有启用配置时，新本地配置先保持停用，在节点页显式选择切换或加入混合模式，不暗改当前代理。输入框和弹窗不拦截快捷键；Clipboard 文本可能含凭据，不输出到日志。
