...
本分支最后核对：2026-09-24。选择性整合上游正式版至 1.0.43：冰蓝土星应用图标、经典土星托盘、Windows DPI 标题栏图标、TLS 分片、VLESS packet_encoding 校验、TUN 副内核防回环、端口残留等待缩短和托盘左键唤窗。保留本分支原有订阅去重/节点覆盖模型、代理与 UI 定制；下行是此前功能的历史记录。
