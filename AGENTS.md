...
- `meow.rs` — ★ meow 生成器（Clash YAML，`serde_yaml::Mapping` 保序；字段名以自家 `subscription/clash.rs` 解析器为逆向权威）：ss(+obfs/v2ray-plugin)/vmess/vless(REALITY+Vision)/trojan/hysteria2/anytls/snell/socks5/http + ws/grpc/h2/httpupgrade 传输。主组 `proxy`（select，选中节点排首位）保持 sing-box 的 Clash API 契约（热切 `PUT /proxies/proxy`）；kernel 自动选路 = select 包一层 url-test `auto` 组；filter/smart 池各自 url-test 组；内置 3 条映射 `GEOSITE,cn`/`GEOIP,cn`/`GEOSITE,geolocation-!cn`；bypass_lan 用显式私有 CIDR（MaxMind mmdb 无 private 类别；198.18/15 假 IP 段除外）；block_quic 用 AND 逻辑规则；DNS 走 nameserver/fallback/nameserver-policy(+.suffix/geosite:cn)+hosts+fake-ip；**TUN 强制 fake-ip**（meow 要求）；extra_inbounds 走 `listeners`。**vmess 仅 tcp/ws**（其余传输与 ss+shadow-tls、不支持协议节点在生成期跳过并告警）。用户自建 `.srs` 集跳过。
...
17. **meow 特有约定** — ① Clash YAML 配置写 `config/active.yaml`（JSON 系共用 active.json，互不混用）；启动参数 `-f <abs> -d <data>/meow`（`-d` 同时是相对路径基准，config 必须绝对路径）。② geodata 在 `<data>/meow/`：`Country.mmdb`（MaxMind）+ `geosite.dat`（MetaCubeX .mrs）——**与 Xray 的 bin/geosite.dat 同名不同格式，绝不能共目录**；缺失时 meow 自带的下载会经由未启动的代理 dial 而失败，GEOIP/GEOSITE 规则直接让内核退出，故启动前必须 `ensure_meow_geodata`。③ **vmess 仅 tcp/ws 传输**（生成期跳过其他组合）；TUN 强制 fake-ip DNS（生成器按 BuildOptions.tun_enabled 联动，不回写用户设置）；Windows TUN 用 `bin/meow-wintun.dll`（`MEOW_WINTUN_DLL` 指向，避免与 Xray 的 wintun.dll 相互覆盖）。④ Clash API 全兼容：热切节点/连接监控/delay 测速/智能切换与 sing-box 同款复用（组名 `proxy`、kernel 模式 url-test 组名也是 `proxy`）。⑤ **meow 无 `system` DNS 解析器**（mihomo 有但 meow 未实现，未知 scheme 直接硬错误 `nameserver-policy entry has no valid nameservers`）——DNS Local 分类与 dns_final=local 一律映射为 223.5.5.5 明文 UDP（`dns_never_emits_system_resolver` 单测守护）。
