...
//! slower otherwise, and backs off for TUN-sized snapshots.
...
/// Fast rate: catch short-lived conns while a connection-data page is active.
const WS_INTERVAL_ACTIVE_MS: u64 = 100;
/// Idle rate: no connection-data consumer on screen.
...
/// Re-evaluate the URL interval at most this often; changing it reconnects
/// the WebSocket and should not flap on every page transition/tier edge.
const INTERVAL_RECHECK: Duration = Duration::from_secs(2);
/// TUN sees every machine connection; large JSON frames need a lower sample
/// rate to keep the Rust heap and WebView responsive.
...
fn interval_for(visible: bool, demand: bool, last_frame_connections: usize) -> u64 {
    let base = if visible && demand {
        WS_INTERVAL_ACTIVE_MS
    } else {
        WS_INTERVAL_BACKGROUND_MS
    };
    let tun_floor = if last_frame_connections > TUN_TIER2_CONNS {
        TUN_TIER2_MS
    } else if last_frame_connections > TUN_TIER1_CONNS {
        TUN_TIER1_MS
    } else {
        WS_INTERVAL_ACTIVE_MS
    };
...
mod tests {
    ...
    fn fast_rate_needs_visibility_and_demand() {
        assert_eq!(interval_for(true, true, 0), WS_INTERVAL_ACTIVE_MS);
    ...
    #[test]
    fn large_tun_frames_back_off_sampling() {
        assert_eq!(
