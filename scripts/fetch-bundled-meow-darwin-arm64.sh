#!/usr/bin/env bash
# Download official meow (macOS Apple Silicon) into app resources for
# bundling. Stages meow + meow-version.txt + meow-geodata/ (Country.mmdb +
# mrs geosite.dat from MetaCubeX — the same source meow auto-downloads).
# Usage: scripts/fetch-bundled-meow-darwin-arm64.sh [version]
set -euo pipefail
